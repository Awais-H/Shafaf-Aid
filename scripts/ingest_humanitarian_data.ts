...
async function main() {
    ...
    for (const country of CONFIG.countries) {
        ...
        presenceRows.forEach(r => {
            ...
            // Process Orgs
            const orgName = r['org_name'] || r['organization'] || r['partner'];
            const orgType = r['org_type_name'] || r['org_type'] || 'INGO';
            if (orgName) {
                const orgId = `org_${orgName.toLowerCase().replace(/[^a-z0-9]/g, '_').substring(0, 30)}`;
                if (!allOrgs.has(orgId)) {
                    allOrgs.set(orgId, { id: orgId, name: orgName, type: orgType });
                }

                // Process Edges
                const sector = r['sector_name'] || r['sector'] || 'General';
                ...
                allEdges.push({
                    id: `edge_${allEdges.length}`, orgId: `org_${orgName.toLowerCase().replace(/[^a-z0-9]/g, '_').substring(0, 30)}`, regionId, aidType, projectCount: 1, isSynthetic: false, source: 'hdx_hapi'
                });
            }
        });

...
/**
 * Loads static data from JSON files generated by ingestion script
 */
export async function loadStaticData(): Promise<AppData> {
  ...
  try {
    // Fetch generated JSONs from public/data/
    const [countries, regions, organizations, aidEdges] = await Promise.all([
      ...
      fetch('/data/regions.json').then(r => r.json() as Promise<Region[]>),
      fetch('/data/orgs.json').then(r => r.json() as Promise<Organization[]>),
      fetch('/data/aid_edges.json').then(r => r.json() as Promise<AidEdge[]>),
    ]);
    ...
    console.error('Failed to fetch generated data:', error);
    // Fallback to empty state or throw - likely means script hasn't run
    throw new Error('Failed to load humanitarian data. Please run the ingestion script.');
  }
}
